...
func TestProxyURLForAccount_Resin(t *testing.T) {
	...
	proxy := Proxy{
		...
		Host:     "127.0.0.1",
		Port:     2260,
		Username: "Default.<accountid>",
		Password: "my-token",
	}

	if got := proxy.URL(); got != "socks5h://Default.%3Caccountid%3E:my-token@127.0.0.1:2260" {
...
func TestProxyURLForAccount_CaseInsensitivePlaceholder(t *testing.T) {
	t.Parallel()
